...
( function ( root , factory ) {
  ...
  if ( Object.create ) {
    ...
    ( function () {
      ...
      function Surrogate() {} // eslint-disable-line func-style, no-empty-function
  ...
  is.use = function use( bundle ) {
    ...
    if ( typeof bundle === 'function' ) {
      ...
      throw new TypeError( 'bundle must be a function' )
    }
  ...
  is.use( function numberBundle( util ) {
    ...
    //
    // __is.numeric( value )__
    //
    // Checks whether given value is likely a number.
    //

    util.addPredicate( 'numeric' , function isNumeric( value ) {
      ...
      return is.not.infinite( value ) && is.not.nan( value )
    ...
    if ( Number.isInteger ) {
      ...
      util.addPredicate( 'integer' , function isInteger( number ) {
        return is.finite( number ) && floor( number ) === number
      } )
    }
  ...
  is.defaults = {
    ...
    // Whether or not to skip holes when iterating over sparse arrays.
    skipHoles : true ,
