describe( 'bundle:object' , function () {
    ...
    it( 'is.emptyObject' , function () {
        ...
        if ( supportsDefineProperty ) {
            ...
            expect( is.emptyObject( Object.defineProperty( {} , 'foo' , { value : 'bar' , enumerable : false } ) ) ).toBeTruthy()

            if ( typeof Symbol === 'function' ) {
                // ignore non-string-keyed properties
                expect( is.emptyObject( Object.defineProperty( {} , Symbol() , { value : 'bar' , enumerable : true } ) ) ).toBeTruthy()
            }
    ...
    it( 'is.conforms' , function () {
        ...
        expect( is.conforms( { foo : 0 , bar : 1 } , { foo : is.string } ) ).not.toBeTruthy()

        expect( is.conforms( { foo : 0 , bar : 1 } , { foo : is.number } ) ).toBeTruthy()

        expect( is.conforms( { foo : 0 , bar : 1 } , { foo : is.number , bar : is.number } ) ).toBeTruthy()

        expect( is.conforms( { foo : 0 , bar : 1 } , { foo : is.number , bar : is.string } ) ).not.toBeTruthy()

        expect( is.conforms( { foo : 0 , bar : 1 } , { foo : is.string , bar : is.number } ) ).not.toBeTruthy()

        expect( is.conforms( { foo : 0 , bar : 1 } , { foo : is.string , bar : is.string } ) ).not.toBeTruthy()

        expect( is.conforms( { foo : 0 , bar : 1 } , { foo : is.number , bar : is.number , baz : is.number } ) ).not.toBeTruthy()
        ...
        expect( is.conforms( { foo : 0 , bar : 1 } , { foo : is.string } , true ) ).not.toBeTruthy()

        expect( is.conforms( { foo : 0 , bar : 1 } , { foo : is.number } , true ) ).not.toBeTruthy()

        expect( is.conforms( { foo : 0 , bar : 1 } , { foo : is.number , bar : is.number } , true ) ).toBeTruthy()

        expect( is.conforms( { foo : 0 , bar : 1 } , { foo : is.number , bar : is.string } , true ) ).not.toBeTruthy()

        expect( is.conforms( { foo : 0 , bar : 1 } , { foo : is.string , bar : is.number } , true ) ).not.toBeTruthy()

        expect( is.conforms( { foo : 0 , bar : 1 } , { foo : is.string , bar : is.string } , true ) ).not.toBeTruthy()

        expect( is.conforms( { foo : 0 , bar : 1 } , { foo : is.number , bar : is.number , baz : is.number } , true ) ).not.toBeTruthy()
